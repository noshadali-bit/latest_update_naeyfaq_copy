                     ...
                     <table id="example" class="display table dataTable table-striped table-bordered">
                        ...
                        <tbody>
                           ...
                           <tr>
                              ...
                              <td>{{$val->title}}</td>
                              <td>{{ Str::limit($val['description'], 120) }}</td>
                              <td>{{$val->name}}</td>
                              <td>{{$val->email}}</td>

                              <td>
                                 <select name="active_blog" id="active_blog" class="form-control active_blog" edit_id="{{$val->id}}">
...
<script type="text/javascript">
   $(".active_blog").change(function(){
      
      $.ajax({
        method: "POST",
        url: "{{ route('active_blog') }}",
        data: {
          "_token": "{{ csrf_token() }}",
